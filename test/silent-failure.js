...
const pageServer = await listen((req, res) => {
  const endpoint = new URL(req.url, "http://127.0.0.1").searchParams.get("endpoint");
  res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
  res.end(pageHtml(endpoint));
}, pagePort);

async function runCase(browser, name, endpointServer) {
  ...
  await page.goto("http://127.0.0.1:" + pagePort + "/?endpoint=" + encodeURIComponent(endpoint));
